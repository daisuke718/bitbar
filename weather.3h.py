...
core_location_cli_path = '/usr/local/bin/CoreLocationCLI'

def mac_location_lookup():
  try:
    exit_code, loc = commands.getstatusoutput(core_location_cli_path + ' -once yes -format "%latitude,%longitude"')
    if exit_code != 0:
      raise ValueError('CoreLocationCLI not found')
    formatted_city_name, city_name = reverse_latlong_lookup(loc)
    return { "loc": loc, "preformatted": formatted_city_name, "city_name": city_name }
  except:
    return False
    ...
    if 'results' in location:
      for component in location['results'][0]['address_components'] :
        if 'locality' in component['types'] :
          ...
          break;
      formatted_city_name = location['results'][0]['formatted_address'].encode('UTF-8')
      return formatted_city_name, city_name
    else:
      return 'Could not lookup location name(Cannot get result)', 'Could not lookup location name'
  except:
    return 'Could not lookup location name(Exception occurred)', 'Could not lookup location name'
...
def get_wx():
  ...
  try:
    ...
    if 'currently' in wx:
      for item in wx['currently']:
        if item == 'temperature':
          ...
          weather_data['color'] = get_color(str(wx['currently']['icon']))
        elif item == 'summary':
    ...
    if 'preformatted' in location:
      ...
      weather_data['city_name'] = location['city_name']
...
def render_wx():
  ...
  if 'city' in weather_data and 'region' in weather_data:
    ...
    print weather_data['city_name']  + ' | href=https://darksky.net/' + weather_data['loc']  
    print weather_data['preformatted'] + ' | href=https://darksky.net/' + weather_data['loc'] + ' alternate=true ' 

  if 'condition' in weather_data and 'feels_like' in weather_data:
    print weather_data['condition'] + ', Feels Like: ' + weather_data['feels_like'] + ' | color=' + weather_data['color']
